...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0,4,2,3,1).contiguous().view(-1,V,C)
        ...
        x = x.view(N,M,T,V,-1).contiguous().permute(0,1,3,4,2).contiguous().view(N,-1,T)
        x = self.data_bn(x)
        # x = rearrange(x, 'n (m v c) t -> (n m) c t v', m=M, v=V).contiguous()
        x = x.view(N,M,V,-1,T).contiguous().permute(0,1,3,4,2).contiguous().view(N*M,-1,T,V)
